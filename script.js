...
const generator = function () {
  ...
  const name = filenameReplace(folderName.value); //呼び出しと同時にやばい記号置換
  ...
  //TSV用の二次元配列、後で反転させる
  let tsvData = [[' ', '1時間目', '2時間目', '3時間目', '4時間目', '5時間目', '6時間目', '7時間目', '8時間目']];

  let weekdays = ['Mo', 'Tu', 'We', 'Th', 'Fr', 'Sa'];//曜日、elementIdを取得する用
  let weekdaysJp = ['1_月曜日', '2_火曜日', '3_水曜日', '4_木曜日', '5_金曜日', '6_土曜日'];//曜日、ファイル名に入れる用
  ...
  weekdays.forEach(function (element, index) {
    ...
    for (let nStudy = 1; nStudy <= 8; nStudy++) {
      ...
      if (nowLesson == "") {
        ...
        console.log(nowPass)
        //各科目用のメモを作成する
        zip.file(`${nowPass}/${nowLesson}_メモ.txt`, `これは${nowLesson}のメモです`)


        //各回分のメモを作成する(使わない可能性高いがフォルダ作るために)
        for (let nTime = 1; nTime <= lessonsNum.value; nTime++) {
          zip.file(`${nowPass}/${nowLesson}_${nTime}回目/${nowLesson}の${nTime}回目メモ.txt`, `これは${nowLesson}の${nTime}回目メモです`);
        }
        //TSVのデータに追加する
